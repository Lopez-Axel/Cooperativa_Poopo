    ...
    <div class="attendance-details-page">
            ...
            <div class="header-content">
                <div class="header-text">
                    ...
                    <p class="subtitle">
                        {{
                            formatDate(
                                periodStore.selectedPeriod.fecha_asistencia,
                            )
                        }}
                    </p>
                </div>
                ...
                <div class="stat-content">
                    <div class="stat-item">
                        ...
                        <span class="stat-value">
                            ...
                            {{
                                periodStore.selectedPeriod.hora_inicio.substring(
                                    0,
                                    5,
                                )
                            }}
                            -
                            {{
                                periodStore.selectedPeriod.hora_fin.substring(
                                    0,
                                    5,
                                )
                            }}
                    ...
                    <div class="stat-item">
                        ...
                        <span
                            v-if="periodStore.selectedPeriod.is_open"
                    ...
                    <div class="attendance-count">
                        ...
                        <div class="count-text">
                            <span class="count-number">{{
                                periodStore.selectedPeriod.total_marked
                            }}</span>
                            <span class="count-divider">/</span>
                            <span class="count-total">{{
                                periodStore.selectedPeriod.total_expected
                            }}</span>
                        </div>
                    </div>
                    <div
                        v-if="periodStore.selectedPeriod.total_expected > 0"
                    ...
                    >
                        ...
                        <td>
                            <strong>{{
                                getCooperativistaById(
                                    attendance.cooperativista_id,
                                )?.nombres || "Cooperativista no encontrado"
                            }}</strong>
                            <br />
                            <small>
                                {{
            ...
            <div class="modal-card">
                ...
                <section class="modal-card-body">
                    <div class="field">
                        ...
                        <div
                            class="control has-icons-left cooperativista-search-wrapper"
                        >
                            <input
                                class="input"
                                type="text"
                                v-model="cooperativistaSearchQuery"
                                @focus="showCooperativistaDropdown = true"
                            ...
                            <div
                                v-if="
                                    showCooperativistaDropdown &&
                                    filteredCooperativistas.length > 0
                                "
                                class="cooperativista-dropdown"
                            >
                                <div
                                    v-for="coop in filteredCooperativistas"
                                    :key="coop.id"
                                    class="cooperativista-item"
                                    @mousedown.prevent="
                                        selectCooperativista(coop)
                                    "
                                >
                                    <span class="coop-ci">{{ coop.ci }}</span>
                                    <span class="coop-name"
                                        >{{ coop.nombres }}
                                        {{ coop.apellido_paterno }}
                                        {{ coop.apellido_materno }}</span
                                    >
                                </div>
                            </div>
                            <div
                                v-if="
                                    showCooperativistaDropdown &&
                                    cooperativistaSearchQuery &&
                                    filteredCooperativistas.length === 0
                                "
                                class="cooperativista-dropdown empty"
                            >
                    ...
                    <div class="field">
                        <label class="label"
                            >Motivo del Registro Manual *</label
                        >
                        <div class="control has-icons-left">
                            <div class="select is-fullwidth">
                                <select v-model="manualAttendanceForm.reason">
                                    ...
                                    <option value="Falta de credencial">
                                        Falta de credencial
                                    </option>
                                    <option value="Registro tardío">
                                        Registro tardío
                                    </option>
                                    <option value="Credencial dañada">
                                        Credencial dañada
                                    </option>
                                    <option value="Error en scanner">
                                        Error en scanner
                                    </option>
                    ...
                    <div
                        v-if="manualAttendanceForm.reason === 'Otro'"
                        class="field"
                    >
                    ...
                    <div class="notification is-info">
                        <p>
                            <strong>Nota:</strong> Se creará un log con el
                            motivo especificado para auditoría.
                        </p>
                    </div>
                </section>
                <footer class="modal-card-foot">
                    <button
                        ...
                        :disabled="
                            !canSaveManualAttendance || attendanceStore.loading
                        "
                    >
                    ...
                    <button class="button" @click="closeAddModal">
                        Cancelar
                    </button>
                </footer>
            </div>
        ...
        <div class="modal" :class="{ 'is-active': showLogsModal }">
            ...
            <div class="modal-card">
                ...
                <section class="modal-card-body">
                    <div
                        v-if="attendanceStore.logs.length === 0"
                        class="empty-state"
                    >
                ...
                <footer class="modal-card-foot">
                    <button class="button" @click="showLogsModal = false">
                        Cerrar
                    </button>
                </footer>
            </div>
        ...
        <div class="modal" :class="{ 'is-active': showDeleteModal }">
            ...
            <div class="modal-card">
                ...
                <footer class="modal-card-foot">
                    ...
                    >
                        <i class="mdi mdi-delete"></i>
                        <span v-if="attendanceStore.loading"
                            >Eliminando...</span
                        >
...
// Propiedades para el motor de búsqueda de cooperativistas
const cooperativistaSearchQuery = ref("");
const showCooperativistaDropdown = ref(false);
...
const filteredCooperativistas = computed(() => {
    if (!cooperativistaSearchQuery.value.trim()) {
        return activeCooperativistas.value; // Mostrar todos cuando no hay búsqueda
    }

    const query = cooperativistaSearchQuery.value.toLowerCase().trim();
    return activeCooperativistas.value.filter((coop) => {
        const nombreCompleto =
            `${coop.nombres} ${coop.apellido_paterno} ${coop.apellido_materno}`.toLowerCase();
        const ci = coop.ci ? coop.ci.toLowerCase() : "";
...
const canSaveManualAttendance = computed(() => {
    const hasReason =
        ...
        (manualAttendanceForm.value.reason !== "Otro" ||
...
// Métodos para el motor de búsqueda de cooperativistas
const selectCooperativista = (cooperativista) => {
    manualAttendanceForm.value.cooperativista_id = cooperativista.id;
    const apellidoPaterno = cooperativista.apellido_paterno || "";
    const apellidoMaterno = cooperativista.apellido_materno || "";
    cooperativistaSearchQuery.value =
        `${cooperativista.ci} - ${cooperativista.nombres} ${apellidoPaterno} ${apellidoMaterno}`.trim();
    showCooperativistaDropdown.value = false;
};

const hideDropdownWithDelay = () => {
    // Delay para permitir que el click en un item se procese antes de ocultar
    setTimeout(() => {
        showCooperativistaDropdown.value = false;
    }, 200);
};

// Watch para resetear la búsqueda cuando se cierra el modal
...
/* Estilos para el motor de búsqueda de cooperativistas */
.cooperativista-search-wrapper {
    position: relative;
}

.cooperativista-dropdown {
    position: absolute;
    top: 100%;
    left: 0;
    right: 0;
...
.cooperativista-item:last-child {
    border-bottom: none;
}

.cooperativista-item .coop-ci {
    font-weight: 700;
    color: #ffd700;
    min-width: 90px;
...
.cooperativista-item .coop-name {
    color: #c8e6c9;
    flex: 1;
}

.cooperativista-dropdown .empty-message {
    padding: 1.5rem;
    text-align: center;
    color: #90a4ae;
    display: flex;
...
.cooperativista-dropdown .empty-message i {
    font-size: 1.5rem;
    color: #607d8b;
}

.cooperativista-dropdown::-webkit-scrollbar {
    width: 8px;
}

.cooperativista-dropdown::-webkit-scrollbar-track {
    background: rgba(255, 255, 255, 0.05);
    border-radius: 4px;
}

.cooperativista-dropdown::-webkit-scrollbar-thumb {
    background: rgba(255, 215, 0, 0.3);
    border-radius: 4px;
}
